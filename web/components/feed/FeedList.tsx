...
import { Clock, Compass, Coffee, RefreshCw, Star } from 'lucide-react';
...
export default function FeedList() {
  ...
  const categoryCounts = useMemo(() => {
    ...
    notes.forEach((post) => {
      const text = `${post.title || ''} ${post.content || ''} ${post.body || ''} ${(post.hashtags || []).join(' ')}`.toLowerCase();
      if (text.includes('coffee') || text.includes('espresso') || text.includes('roast') || text.includes('brew')) counts['specialty-coffee']++;
      if (text.includes('code') || text.includes('dev') || text.includes('work') || text.includes('laptop')) counts['tech-code']++;
      if (text.includes('outlet') || text.includes('plug') || text.includes('power') || text.includes('socket')) counts['power-outlets']++;
      if (text.includes('quiet') || text.includes('silent') || text.includes('focus') || text.includes('calm')) counts['quiet-focus']++;
      if (text.includes('study') || text.includes('book') || text.includes('read') || text.includes('desk')) counts['books-study']++;
      if (text.includes('food') || text.includes('bakery') || text.includes('croissant') || text.includes('pastry') || text.includes('meal')) counts['food-bakery']++;
      if (text.includes('wifi') || text.includes('wi-fi') || text.includes('internet') || text.includes('fiber') || text.includes('mbps')) counts['fast-wi-fi']++;
      if (text.includes('night') || text.includes('late') || text.includes('24')) counts['open-late']++;
      if (text.includes('garden') || text.includes('outdoor') || text.includes('terrace') || text.includes('patio')) counts['outdoor']++;
      if ((post.likes_count ?? post.post_likes?.length ?? 0) > 0) counts['coffee-vote']++;
      if (text.includes('cozy') || text.includes('vibe') || text.includes('chill')) counts['cozy-vibe']++;
      if (text.includes('budget') || text.includes('cheap') || text.includes('affordable')) counts['budget']++;
    });
  ...
  const filteredPosts = useMemo(() => {
    ...
    if (selectedCity && selectedCity !== 'All Locations') {
      ...
      result = result.filter((post) => {
        ...
        return address.includes(cityKeyword) || content.includes(cityKeyword);
      });
    }
        ...
        switch (selectedCategory) {
          case 'specialty-coffee':
            return (
              ...
              tags.some((t) => t.includes('coffee') || t.includes('specialty'))
            );
          case 'tech-code':
            return (
              title.includes('code') ||
              ...
              tags.some((t) => t.includes('code') || t.includes('tech'))
            );
          case 'fast-wi-fi':
            return (
          ...
          case 'power-outlets':
            return (
              ...
              tags.some((t) => t.includes('outlet') || t.includes('plug'))
            );
          case 'quiet-focus':
            return (
              ...
              tags.some((t) => t.includes('quiet') || t.includes('focus'))
            );
          case 'books-study':
            return (
          ...
          case 'open-late':
            return (
              ...
              content.includes('24') ||
              tags.some((t) => t.includes('late') || t.includes('night'))
            );
          case 'outdoor':
            return (
              ...
              tags.some((t) => t.includes('garden') || t.includes('outdoor'))
            );
          case 'coffee-vote':
            return (post.likes_count ?? post.post_likes?.length ?? 0) > 0;
          default:
            return true;
        }
      });
  ...
  return (
    <div className="w-full pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {notes && notes.length > 0 && !searchQuery && selectedCategory === 'all' && (
        ...
        <div className="pt-6 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.06] mb-6">
          ...
          <div className="flex items-center gap-2 text-xs text-zinc-400">
            {filteredPosts.length > 0 && (
              <span>
                Showing <strong className="text-white">{filteredPosts.length}</strong> {filteredPosts.length === 1 ? 'place' : 'places'}
              </span>
            )}
            {(searchQuery || selectedCategory !== 'all' || selectedCity !== 'All Locations') && (
              <button
                onClick={() => {
                  setSearchQuery('');
        ...
        {notes !== null && filteredPosts.length === 0 && (
          <div className="text-center py-20 px-4 rounded-3xl border border-dashed border-white/10 bg-[#18181b]/40 my-6">
            ...
            <p className="text-xs text-zinc-400 max-w-md mx-auto mb-6">
              We couldn’t find any spaces matching your criteria. Try adjusting your filters or search keywords.
            </p>
            <button
              onClick={() => {
                setSearchQuery('');
